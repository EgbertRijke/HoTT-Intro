...
\item \label{ex:equiv_precomp}Let $f:A\to B$ be a map. Show that the following are equivalent:
\begin{enumerate}
...
\end{equation*}
\begin{subexenum}
\item Show that the type $\sm{i:A\to B}h\circ f\htpy g\circ i$ is equivalent to the type of fiberwise transformations
\begin{equation*}
