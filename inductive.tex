...
\end{equation*}
satisfying the computation rule\index{compution rules!of Sigma types@{of $\Sigma$-types}}
\begin{equation*}
...
\begin{equation*}
\proj 1\defeq \ind{\Sigma}(\lam{x}{y}x).
\end{equation*}
...
\begin{equation*}
\proj 2\defeq \ind{\Sigma}(\lam{x}{y}y).
\end{equation*}
...
\begin{tabular}{lll}
\toprule
Premises & Type encoding in $\UU$ & Type of elements $\mathrm{El}(\blank)$ \\
\midrule
...
\begin{defn}
We define the type family $\mathsf{Fin}:\N\to\UU$ of finite types by induction on $\N$\index{family!of finite types}, taking
\begin{align*}
\mathsf{Fin}(0) & \defeq \emptyt \\
\mathsf{Fin}(n+1) & \defeq \mathsf{Fin}(n)+1
\end{align*}
\end{defn}

\begin{defn}\label{defn:obs_nat}
We define the \define{observational equality}\index{observational equality!on N@{on $\N$}} on $\N$ as binary relation $\mathrm{Eq}_\N:\N\to(\N\to\UU)$ satisfying
\begin{align*}
...
\begin{eg}
Another important way to use the universe is to define types of \define{structured types}\index{structured types}. We give some examples:
\begin{enumerate}
...
In the following lemma we derive an alternative induction principle\index{induction principle!of Z@{of $\Z$}} for $\Z$, which makes it easer to make definitions.
\begin{lem}
\label{lem:Z_ind}
...
\item \label{ex:obs_nat_eqrel}Show that observational equality on $\N$\index{observational equality!on N@{on $\N$}} is an equivalence relation, i.e.~construct terms of the following types:
\begin{align*}
...
\end{align*}
\item \label{ex:obs_nat_least}\index{observational equality!on N@{on $\N$}}Let $R$ be a reflexive binary relation\index{reflexive relation} on $\N$, i.e.~$R$ is of type $\N\to (\N\to\UU)$ and comes equipped with a term $\rho:\prd{n:\N}R(n,n)$. Show that
\begin{equation*}
\prd{n,m:\N} \mathrm{Eq}_\N(n,m)\to R(n,m).
\end{equation*}
\item \index{observational equality!on N@{on $\N$}}Show that every function $f:\N\to \N$ preserves observational equality in the sense that
\begin{equation*}
...
\item Show that $\leq$ is reflexive and that $<$ is \define{anti-reflexive}, i.e. that $\neg(n<n)$. 
\item Show that both $\leq$ and $<$ are transitive, and that $n<S(n)$. 
\end{subexenum}
\item \index{observational equality!on N@{on $\N$}}Use the observational equality of the natural numbers to define the divisibility relation $d\mid n$. 
...
\begin{subexenum}
\item Define the relations $\leq$ and $<$ on and $\Z$.
