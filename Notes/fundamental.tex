...
\begin{proof}
By \autoref{thm:fib_equiv} it follows that the family of maps $\mathsf{path\usc{}ind}_a(b)$ is a family of equivalences if and only if it induces an equivalence
\begin{equation*}
...
\begin{subexenum}
\item Use \autoref{ex:htpy_total,ex:contr_retr} to show that if each $B(x)$ is a retract of $\id{a}{x}$, then $B(x)$ is equivalent to $\id{a}{x}$ for every $x:A$.
