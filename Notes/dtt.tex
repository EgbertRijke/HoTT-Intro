...
\section{Weakening, substitution, and the variable rule}
...
The idea of the derivation is as follows: If we have a judgment
\begin{equation*}
...
\end{prooftree}
\item Show that variables can be changed, i.e., give a derivation of the following inference rule.
  \begin{prooftree}
    \AxiomC{$\Gamma,x:A,\Delta\vdash \mathcal{J}$}
    \UnaryInfC{$\Gamma,x':A,\Delta[x'/x]\vdash \mathcal{J}[x'/x]$}
  \end{prooftree}
