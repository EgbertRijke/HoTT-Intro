...
\end{tikzcd}
\end{equation*}
for every $x:A$. [commuting triangle]. We note that the top map is an equivalence, so it follows by the 3-for-2 property of equivalences that the left map is an equivalence if and only if the right map is an equivalence. 
...
\end{tikzcd}
\end{equation*}
is a pullback square. This is the \emph{descent property} of the empty type.\index{descent!empty type}
\end{exercises}
