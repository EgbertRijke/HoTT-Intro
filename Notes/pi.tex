...
Given $n:\N$ and a function $f:\N\to\N$ we define $\mathsf{add\usc{}}\succN(n,f)$ to be the function $\lam{m}\succN(f(m))$. This function simply adds one to $f$ pointwise (and it ignores $n$). The idea is that if $f$ represents adding $m$, then $\mathsf{add\usc{}}\succN(n,f)$ should be adding one more than $f$ did.
\end{proof}
...
\UnaryInfC{$\vdash \mathsf{add\usc{}}\succN\defeq \lam{n}{f}{m}\succN(f(m)):\N\to (\N^\N \to \N^\N)$}
