...
\end{equation*}
is an equivalence. For notational brevity we will just write $\mathsf{gen}_{\mathcal{S}}$\index{gen_S@{$\mathsf{gen}_{\mathcal{S}}$}} for $\mathsf{cocone\usc{}map}_{\mathcal{S}}(\inl,\inr,\glue)$, because $\mathsf{cocone\usc{}map}_{\mathcal{S}}(\inl,\inr,\glue)$ is just the action on generators.
...
\subsection{Using the duality of pullbacks and pushouts}
