...
\end{tikzcd}
\end{equation*}
We call such a triple $\mathcal{S}\jdeq (S,f,g)$ a \define{span} from $A$ to $B$.
...
\begin{defn}
Consider a span $\mathcal{S}\jdeq (S,f,g)$ from $A$ to $B$, and let $P$ be a family over $A\sqcup^{\mathcal{S}} B$. The \define{dependent action on generators} is defined to be the map
\begin{align*}
...
\begin{defn}
Given a span $\mathcal{S}\jdeq (S,f,g)$, the \define{(homotopy) pushout} $A\sqcup^{\mathcal{S}} B$ of $\mathcal{S}$ is defined to be the higher inductive type equipped with
\begin{align*}
\inl & : A \to A \sqcup^{\mathcal{S}} B \\
\inr & : B \to A \sqcup^{\mathcal{S}} B \\
\glue & : \prd{x:S} \inl(f(x))=\inr(g(x)),
\end{align*}
such that for each type family $P$ over $A\sqcup^{\mathcal{S}} B$ the map $\mathsf{dgen}_{\mathcal{S}}^P$ has a section.
\end{defn}
...
\begin{defn}
We define the \define{$n$-sphere} $\sphere{n}$ for any $n:\N$ by induction on $n$, by taking
\begin{align*}
\sphere{0} & \defeq \bool \\
...
\begin{defn}
Given a map $f:A\to B$, we define the \define{cofiber} $\mathsf{cofib}_f$ of $f$ as the pushout
\begin{equation*}
\begin{tikzcd}
...
\end{tikzcd}
\end{equation*}
The cofiber of a map is sometimes also called the \define{mapping cone}.
\end{defn}
...
\begin{defn}
Suppose $A$ and $B$ are pointed types, with base points $a_0$ and $b_0$, respectively. The \define{(binary) wedge} $A\vee B$ of $A$ and $B$ is defined as the pushout
\begin{equation*}
\begin{tikzcd}
...
\begin{defn}
Given a type $I$, and a family of pointed types $A$ over $i$, with base points $a_0(i)$. We define the \define{(indexed) wedge} $\bigvee_{(i:I)}A_i$ as the pushout
\begin{equation*}
...
\end{tikzcd}
\end{equation*}
with $H:i\circ f \htpy j\circ g$ is said to be a \define{(homotopy) pushout square} if the cocone $(i,j,H)$ with vertex $X$ on the span $\mathcal{S}\jdeq (S,f,g)$
satisfies the \define{universal property of pushouts}, which asserts that the map
\begin{equation*}
\mathsf{cocone\usc{}map}(i,j,H):(X\to Y)\to \mathsf{cocone}(Y)
\end{equation*}
is an equivalence for any type $Y$. Sometimes pushout squares are also called \define{cocartesian squares}.
\end{defn}
...
\begin{equation*}
(A\sqcup^{\mathcal{S}} B \to X)\to \mathsf{cocone}_{\mathcal{S}}(X)
\end{equation*}
is an equivalence. To see this, we first show that there is a commuting triangle
\begin{equation*}
\begin{tikzcd}[column sep=0]
& X^{A\sqcup^{\mathcal{S}} B} \arrow[dl,swap,"\mathsf{cocone\usc{}map}_{\mathcal{S}}"] \arrow[dr,"\mathsf{dgen}_{\mathcal{S}}"] \\
...
\end{align*}
The bottom equivalence is the induced map on total spaces of the fiberwise equivalence
\begin{equation*}
(p:f'(f(x))=g'(g(x)))\mapsto \ct{\mathsf{tr\usc{}triv}(\glue(x),f'(f(x)))}{p}.
\end{equation*}
\end{proof}
\end{comment}
...
\begin{subexenum}
\item Show that $\join{P}{Q}$ is again a proposition.
