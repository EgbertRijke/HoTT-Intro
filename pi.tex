...
\end{prooftree}
The idea of dependent function types is that their terms are dependent functions, i.e.~constructions that provide for every $x:A$ a term $b(x):B(x)$. Thus, dependent functions are formed from terms $b(x)$ of type $B(x)$ in context $\Gamma,x:A$, i.e.~the \define{$\lambda$-abstraction rule}\index{lambda-abstraction@{$\lambda$-abstraction}}\index{rule!lambda-abstraction@{$\lambda$-abstraction}} is as follows:
\begin{prooftree}
