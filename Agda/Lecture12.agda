...
IND-circle :
  ...
  UU ((lsuc l2) ⊔ l1)
IND-circle l2 {X} l = (P : X → UU l2) → sec (ev-free-loop' l P)
...
tr-Eq-subst f g refl q .(concat (g _) (ap (λ b → b) q) refl) refl =
  inv (ap-id q) ∙ (inv (right-unit _))
...
isretr-ind-circle :
  ...
  (ind-circle : IND-circle l2 l) (P : X → UU l2) →
...
dependent-universal-property-circle-ind-circle :
  {l1 : Level} (l2 : Level) {X : UU l1} (l : free-loops X) →
  IND-circle l2 l → dependent-universal-property-circle l2 l
dependent-universal-property-circle-ind-circle l2 l ind-circle P =
  is-equiv-has-inverse'
    ( pr1 (ind-circle P))
    ( pr2 (ind-circle P))
    ( isretr-ind-circle l ind-circle P)

{- We define a type of circles in UU l1 that eliminate into UU l2. -}

CIRCLE :
  (l1 l2 : Level) → UU ((lsuc l1) ⊔ (lsuc l2))
CIRCLE l1 l2 =
  Σ (UU l1) (λ X →
    Σ (free-loops X) (IND-circle l2))
...
universal-property-circle :
  {l1 l2 : Level} (Circle : CIRCLE l1 l2) (Y : UU l2) →
  is-equiv (ev-free-loop (pr1 (pr2 Circle)) Y)
universal-property-circle (dpair circle (dpair l ind-circle)) Y =
  is-equiv-right-factor
    ...
    ( is-equiv-comparison-free-loops l Y)
    ( dependent-universal-property-circle-ind-circle _ l ind-circle (λ x → Y))
