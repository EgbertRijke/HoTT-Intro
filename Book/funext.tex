...
\begin{defn}
  ...
  \end{equation*}
  that sends $\refl{f}$ to $\htpyrefl_{f}$ is an equivalence. We will write $\eqhtpy$\index{eq-htpy@{$\eqhtpy$}} for its inverse, if it is assumed to exist.
\end{defn}
...
\end{subexenum}
\item Show that $\pathsplit(f)$\index{path-split!is a proposition} and $\iscohinvertible(f)$\index{coherently invertible!is a proposition} are propositions for any map $f:A\to B$. Conclude that we have equivalences\index{is-equiv(f)@{$\isequiv(f)$}!is-equiv(f) path-split(f)@{$\isequiv(f)\eqvsym\pathsplit(f)$}}\index{is-equiv(f)@{$\isequiv(f)$}!is-equiv(f) is-coh-invertible(f)@{$\isequiv(f)\eqvsym\iscohinvertible(f)$}}
  \begin{equation*}
