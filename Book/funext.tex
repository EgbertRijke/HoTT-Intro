...
\begin{defn}
  The \define{axiom of function extensionality}\index{function extensionality}\index{identity type!Pi-type@{$\Pi$-type}}\index{extensionality principle!for functions} asserts that for any type family $B$ over $A$, and any two dependent functions $f,g:\prd{x:A}B(x)$, the canonical map\index{htpy-eq@{$\mathsf{htpy\usc{}eq}$}}
  \begin{equation*}
...
The type theoretic principle of choice\index{type theoretic choice} asserts that $\Pi$ distributes over $\Sigma$\index{distributivity!Pi over Sigma@{$\Pi$ over $\Sigma$}}. More precisely, it asserts that the canonical map\index{choice@{$\mathsf{choice}$}}
\begin{equation*}
...
\begin{exercises}
\item Show that the functions\index{htpy-inv@{$\mathsf{htpy\usc{}inv}$}}\index{htpy-concat@{$\mathsf{htpy\usc{}concat}$}}\index{htpy-concat'@{$\mathsf{htpy\usc{}concat}'$}}
\begin{align*}
...
\begin{subexenum}
\item Show that for any type $A$ the type $\iscontr(A)$ is a proposition\index{iscontr@{$\iscontr$}!is a proposition}. %There's an easy proof using double singleton induction. This is a nice application of weak funext.
