...
\begin{exercises}
\item Let $X$ be a set. Show that the map\index{equiv-eq@{$\equiveq$}!is a group isomorphism}
  \begin{equation*}
...
\item \label{ex:groupop-embedding}
  ...
  \end{subexenum}
\item Let $f:\hom(G,H)$ be a group homomorphism. Show that $f$ preserves units and inverses, i.e., show that\index{group homomorphism!preserves units and inverses}
  \begin{align*}
    f(e_G) & = e_H \\
    f(x^{-1}) & = f(x)^{-1}.
  \end{align*}
