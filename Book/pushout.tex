...
\begin{defn}
  ...
  \begin{equation*}
    \begin{tikzcd}
      X \arrow[r,"\mathsf{const}_\ttt"] \arrow[d,swap,"\mathsf{const}_\ttt"] & \unit \arrow[d,"{\lam{\ttt}\mathsf{S}}"] \\
      \unit \arrow[r,swap,"\lam{\ttt}\mathsf{N} "] & \susp X
    \end{tikzcd}
  \end{equation*}
  is a pushout square.
\end{defn}
...
Spheres can be presented in type theory as suspensions.
...
A slightly slicker version of the universal property of suspensions can be obtained in the case where $X$ is a pointed type. Recall that a pointed type is a pair $(X,x)$ consisting of a type $X$ and a point $x:X$. The point $x:X$ is called the \define{base point} of the pointed type, and often we don't mention it explicitly.

\begin{defn}
  Given two pointed types $X$ and $Y$, a pointed map from $X$ to $Y$ is a pair $(f,p)$ consisting of a map $f:X\to Y$ and a path $p:f(x)=y$ witnessing that $f$ preserves the base point. We write
  \begin{equation*}
    X\to_\ast Y
  \end{equation*}
  for the type of \define{pointed maps} from $X$ to $Y$.
\end{defn}

The following theorem is also called the adjointness of the suspension and loop space functors.

\begin{thm}
  Let $X$ and $Y$ be pointed types. Then the pointed map
  \begin{equation*}
    (\susp X \to_\ast Y) \to_\ast (X\to \loopspace Y)
  \end{equation*}
  given by $(f,p)\mapsto (\lam{x}\ap{f}{\merid(x)},\cdots)$
\end{thm}

\subsection{The join of two types}
...
\begin{thm}
  For any type $X$, the join $\join{\bool}{X}$ is a suspension of $X$.
\end{thm}
