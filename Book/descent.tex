...
\begin{equation*}
  \mathsf{dep\usc{}cocone\usc{}map}(P):\Big(\prd{x:X}P(x)\Big)\to \mathsf{dep\usc{}cocone}_{(i,j,H)}(P).
\end{equation*}
...
\begin{proof}
  ...
  To see that (iii) implies (iv) we note that $\mathsf{dep\usc{}cocone\usc{}map}(P)$ is an equivalence if and only if the gap map of the square in \cref{eq:dppp2} is an equivalence.

  It is clear that (iv) implies (v), so it remains to show that (v) implies (iv). 
\end{proof}
