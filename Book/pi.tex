...
\end{prooftree}
We will write $g\circ f$\index{g composed with f@{$g\circ f$}} for $\mathsf{comp}(g,f)$\index{comp(g,f)@{$\mathsf{comp}(g,f)$}}.
\end{defn}
...
Composition of functions satisfies the left and right unit laws\index{left unit law|see {unit laws}}\index{right unit law|see {unit laws}}\index{unit laws!of function composition}\index{composition!of functions!unit laws}, i.e., we can derive
\begin{prooftree}
\AxiomC{$\Gamma\vdash f:A\to B$}
