...
\begin{prooftree}
  ...
  \UnaryInfC{$\mathcal{C}$}
\end{prooftree}
Inference rules containin above the horizontal line\index{horizontal line|see {inference rule}} a finite list $\mathcal{H}_1$, $\mathcal{H}_2$, \dots, $\mathcal{H}_n$ of \emph{judgments} for the hypotheses\index{inference rule!hypotheses}, and below the horizontal line a single judgment $\mathcal{C}$ for the conclusion\index{inference rule!conclusion}. The system of dependent type theory is described by a set of such inference rules.
