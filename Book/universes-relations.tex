...
\begin{defn}
  A \define{universe}\index{universe} in type theory is a closed type $\UU$\index{U@{$\UU$}} equipped with a type family $\Ty$\index{Ty@{$\Ty$}} over $\UU$ called the \define{universal family}\index{universal family}\index{family!universal family}, equipped with the following structure:
  \begin{enumerate}
  ...
  \item $\UU$ contains terms $\check{\emptyt},\check{\unit},\check{\N}:\UU$
    ...
    \end{align*}
  \end{enumerate}
  Given a universe $\UU$, we say that a type $A$ in context $\Gamma$ is \define{small}\index{small type} with respect to $\UU$ if it occurs in the universe, i.e., if it comes equipped with a term $\check{A}:\UU$ in context $\Gamma$, for which the judgment
  \begin{equation*}
...
We define the \define{observational equality}\index{observational equality!on N@{on $\N$}} on $\N$ as binary relation $\mathsf{Eq}_\N:\N\to(\N\to\UU)$\index{Eq_N@{$\mathsf{Eq}_\N$}} satisfying
\begin{align*}
...
\item \label{ex:obs_bool}\index{observational equality!on 2@{on $\bool$}}
  \begin{subexenum}
  \item Define observational equality $\mathsf{Eq}_\bool$\index{Eq_bool@{$\mathsf{Eq}_\bool$}} on the booleans.
