...
We define the type family $\mathsf{Fin}:\N\to\UU$ of finite types\index{Fin@{$\mathsf{Fin}$}}\index{finite types} by induction on $\N$\index{family!of finite types}, taking
\begin{align*}
...
\exercise \label{ex:obs_nat_eqrel}Show that observational equality on $\N$\index{observational equality!on N@{on $\N$}!is an equivalence relation} is an equivalence relation\index{equivalence relation!observational equality on N@{observational equality on $\N$}}, i.e., construct terms of the following types:
  ...
  \end{align*}
\exercise \label{ex:obs_nat_least}\index{observational equality!on N@{on $\N$}!is least reflexive relation}Let $R$ be a reflexive binary relation\index{reflexive relation}\index{relation!reflexive} on $\N$, i.e., $R$ is of type $\N\to (\N\to\UU)$ and comes equipped with a term $\rho:\prd{n:\N}R(n,n)$. Show that
  \begin{equation*}
...
\exercise \index{observational equality!on N@{on $\N$}!is preserved by functions}Show that every function $f:\N\to \N$ preserves observational equality in the sense that
  ...
  \begin{equation*}
    \prd{n,m:\N}\prd{f:\N\to\N} \EqN(n,m)\to \EqN(f(n),f(m)),
  \end{equation*}
  and pull out the universal quantification over $f:\N\to\N$ by \cref{ex:swap}.
\exercise 
  \begin{subexenum}
  ...
  \item Show that $\leq$ is reflexive\index{reflexive!leq on N@{$\leq$ on $\N$}}, transtive, and \define{antisymmetric}, i.e., show that $m=n$ whenever both $m\leq n$ and $n\leq m$ hold.
  \item Show that $<$ is \define{irreflexive}\index{irreflexive}\index{relation!irreflexive} ($n \nless n$ for any $n$), and transitive. Furthermore show that $n<\succN(n)$.
...
\exercise
  \begin{subexenum}
  \item \label{ex:strong-induction}Show that $\N$ satisfies \define{strong induction}, i.e., construct for any type family $P$ over $\N$ a function of type
    \begin{equation*}
