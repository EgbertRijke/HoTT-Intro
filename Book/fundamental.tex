...
\begin{defn}
An \define{embedding}\index{embedding} is a map $f:A\to B$ satisfying the property that
\begin{equation*}
...
\begin{thm}
\label{cor:emb_equiv} 
Any equivalence is an embedding.\index{embedding!equivalences are embeddings}\index{equivalence!is an embedding}
\end{thm}
...
\end{equation*}
also has a section. We write $\mathsf{path\usc{}split}(f)$\index{path_split(f)@{$\mathsf{path\usc{}split}(f)$}} for the type
\begin{equation*}
