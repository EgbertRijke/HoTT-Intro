...
f : \prd{x:A}B(x)\to C(x).
\end{equation*}
We define the map\index{total(f)@{$\total{f}$}}
\begin{equation*}
...
\begin{lem}\label{lem:fib_total}
  ...
  there is an equivalence\index{fiber!of total(f)@{of $\total{f}$}}\index{total(f)@{$\total{f}$}!fiber}
  \begin{equation*}
...
\begin{thm}\label{thm:fib_equiv}
  ...
  \index{is an equivalence!total(f) of family of equivalences@{$\total{f}$ of family of equivalences}}
  \index{total(f)@{$\total{f}$}!of family of equivalences is an equivalence}\index{is family of equivalences!if total(f) is an equivalence@{iff $\total{f}$ is an equivalence}}
\begin{enumerate}
...
\begin{defn}
  ...
  \end{equation*}
  where $C$ is a type family over $A$, and $D$ is a type family over $B$. In this situation we also say that $g$ is a \define{family of maps over $f$}. Then we define\index{total f(g)@{$\total[f]{g}$}}
  \begin{equation*}
...
\begin{proof}
  ...
  \begin{equation*}
    \begin{tikzcd}[column sep=0]
      \prd{t:\sm{x:A}B(x)}P(t) \arrow[rr,"\evpair"] \arrow[dr,swap,"{\evpt(a,b)}"] & & \prd{x:A}{y:B(x)}P(x,y) \arrow[dl,"{\lam{h}h(a,b)}"] \\
...
\item Show that\index{embedding!closed under homotopies}
\begin{equation*}
...
\item \label{ex:path-split}We say that a map $f:A\to B$ is \define{path-split}\index{path-split} if $f$ has a section, and for each $x,y:A$ the map
\begin{equation*}
...
\sm{x:X}\fib{f}{x} \arrow[r,"\total{\fibtriangle(h,H)}"] \arrow[d] & \sm{x:X}\fib{g}{x} \arrow[d] \\
A \arrow[r,swap,"h"] & B
\end{tikzcd}
\end{equation*}
