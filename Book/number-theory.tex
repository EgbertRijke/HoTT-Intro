...
Typically we are mostly interested in decidability of propositions. However, we have defined the notion of decidability for general types. Therefore it is the case that the type natural numbers, or indeed any type $A$ that comes equipped with a point $a:A$, is decidable. We have at two reasons for defining decidability in this generality.

At this stage of our development of type theory, where we have not yet discussed function extensionality or propositional truncations, it may be hard to show that some types are propositions. Function extensionality is used, for example, to show that
  \begin{equation*}
    \prd{x:A}P(x)
  \end{equation*}
  is a proposition for any subtype $P$ of $A$. Function extensionality is the subject of \cref{chap:funext}. Moreover, the type
  \begin{equation*}
    \sm{x:A}P(x)
  \end{equation*}
  is only a proposition in very specific circumstances. The \emph{proposition} `there exists an $x:A$ for which $P(x)$ holds' is formed using an operation called the \emph{propositional truncation}. Propositional truncations are introduced in \cref{chap:image}. Nevertheless, it is perfectly doable to develop some elementary number theory and discrete mathematics in dependent type theory without function extensionality or propositional truncations.

  A second reason is that for a general type, the condition that its identity types are decidable is of some interest. We now study such types.
...
\begin{proof}
  We show by induction on $n:\N$ that there is a function
  \begin{equation*}
    P(n)\to \Big(\sm{m:\N}{p:P(m)}\mathsf{is\usc{}minimal}_P(m,p)\Big) 
  \end{equation*}
  for every decidable subset $P$ of $\N$. The base case is trivial, since $\zeroN$ is the least natural number. For the inductive step, suppose that $P(\succN(n))$ holds. Note that $P(0)$ is decidable, so we proceed by case analysis on $P(0)$. If $P(0)$ holds, then we have obtained a minimal element in $P$. If $P(0)$ doesn't hold, then consider the decidable subset $P'$ of $\N$ given by
  \begin{equation*}
    P'(n)\defeq P(\succN(n)).
  \end{equation*}
  Since $P'(n)$ holds we obtain a minimal element in $P'$. Of course, by the assumption that $P(0)$ doesn't hold, the minimal element of $P'$ is also the minimal element of $P$.
\end{proof}
