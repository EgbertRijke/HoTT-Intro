...
\begin{defn}
  We say that a type $A$ is \define{contractible}\index{contractible!type} if it comes equipped with a term of type
  \begin{equation*}
    \iscontr(A) \defeq \sm{c:A}\prd{x:A}c=x.
  \end{equation*}
  Given a term $(c,C):\iscontr(A)$, we call $c:A$ the \define{center of contraction}\index{center of contraction} of $A$, and we call $C:\prd{x:A}c=x$ the \define{contraction}\index{contraction} of $A$.
\end{defn}
...
\begin{defn}
  Suppose $A$ comes equipped with a term $a:A$. Then we say that $A$ satisfies \define{singleton induction}\index{singleton induction} if for every type family $B$ over $A$, the map
  \begin{equation*}
    \mathsf{ev\usc{}pt}:\Big(\prd{x:A}B(x)\Big)\to B(a)
  \end{equation*}
...
\sm{x:A}a=x
\end{equation*}
is contractible.\index{identity type!contractibility of total space}
\end{thm}
...
\begin{defn}
Let $f:A\to B$ be a function, and let $b:B$. The \define{fiber}\index{fiber}\index{homotopy fiber|see {fiber}} of $f$ at $b$ is defined to be the type
\begin{equation*}
...
\begin{defn}
We say that a function $f:A\to B$ is \define{contractible}\index{contractible!map} if there is a term of type
\begin{equation*}
...
Any contractible map is an equivalence.\index{contractible!map!is an equivalence}
\end{thm}
...
Any equivalence is a contractible map.\index{equivalence!is a contractible map}
\end{thm}
