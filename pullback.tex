...
In the following theorem we establish the uniqueness of pullbacks up to equivalence via a \emph{3-for-2 property} for pullbacks\index{pullback!3-for-2 property}\index{3-for-2 property!of pullbacks}.
...
\begin{enumerate}
\item $f$ is an equivalence.\index{equivalence!postcomposition|textit}
...
Pullbacks are not only unique in the sense that any two pullbacks of the same cospan are equivalent, they are \emph{uniquely unique}\index{uniquely uniqueness!of pullbacks} in the sense that 
...
\end{tikzcd}
\end{equation*}
If $g$ is a $k$-truncated map, then so is $p$. In particular, if $g$ is an embedding then so is $p$.\index{truncated!map!pullback of|textit}\index{embedding!pullback of|textit}
\end{cor}
...
\end{subexenum}
\item In this exercise we give an alternative characterization of the notion of $k$-truncated map, compared to \cref{thm:trunc_ap}. Given a map $f:A\to X$ define the \define{diagonal}\index{diagonal!of a map} of $f$ to be the map $\delta_f:A\to A\times_X A$ given by $x\mapsto (x,x,\refl{f(x)})$.
\begin{subexenum}
\item Show that the square\index{action on paths!fibers of}\index{diagonal!of a map!fibers of}
\begin{equation*}
\begin{tikzcd}[column sep=large]
...
\end{enumerate}
\item Show that if\index{pullback!cartesian products of pullbacks}
\begin{equation*}
\begin{tikzcd}
...
\item Let $B$ be a type family over $A$. Show that the square\index{Pi-type@{$\Pi$-type}!as pullback}
\begin{equation*}
\begin{tikzcd}[column sep=6em]
